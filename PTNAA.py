...
import PIL
import io
from paths import bezier_curve_path_2nd, bezier_curve_path_3nd, bezier_curve_path_linear, catmull_rom_interpolation, bspline_interpolation

slim = tf.contrib.slim

tf.flags.DEFINE_string('model_name', 'resnet_v2_152', 'The Model used to generate adv.')

tf.flags.DEFINE_string('attack_method', 'PTNAA', 'The name of attack method.')

tf.flags.DEFINE_string('layer_name','resnet_v2_152/block2/unit_8/bottleneck_v2/','The layer to be attacked.')

tf.flags.DEFINE_string('input_dir', './dataset/images/', 'Input directory with images.')

tf.flags.DEFINE_string('output_dir', './adv/PTNAA/res-v2/', 'Output directory with images.')
...
"""parameter for PTNAA"""
tf.flags.DEFINE_string('path', 'bezier_2nd', 'Path function of integrated gradients')

tf.flags.DEFINE_float('sample_ratio', 0.2, 'Non-uniform Sampling ratio')

tf.flags.DEFINE_float('shape_parameter', 10.0, 'Shape parameter of specific path function')
...
def get_random_point_in_ball(x, a):
    shape = x.shape
    random_point = np.random.randn(*shape)
    random_point /= np.linalg.norm(random_point)
    random_radius = np.random.uniform(0, a)
    random_point *= random_radius
    random_point += x
            ...
            for images,names,labels in utils.load_image(FLAGS.input_dir, FLAGS.image_size,FLAGS.batch_size):
                ...
                for i in range(num_iter):
                    if i==0:
                        ...
                        x_base = np.zeros(shape=images.shape)
                        x_base = image_preprocessing_fn(x_base)

                        images_tmp2 = image_preprocessing_fn(np.copy(images))
                        assert x_base.shape[1:] == (int(FLAGS.image_size), int(FLAGS.image_size), 3)
                        a = float(FLAGS.shape_parameter)

                        path_name = str(FLAGS.path)
                        if 'bezier_linear' in path_name:
                            path = np.array(bezier_curve_path_linear(x_base, images_tmp2, ens=int(FLAGS.ens)))
                        elif 'bezier_2nd' in path_name:
                            h = get_random_point_in_ball(images_tmp2, a)
                            path = np.array(bezier_curve_path_2nd(x_base, images_tmp2, h, lately_starting=True, ens=int(FLAGS.ens)))
                        elif 'bezier_3nd' in path_name:
                            p = get_random_point_in_ball(x_base, a)
                            q = get_random_point_in_ball(images_tmp2, a)
                            path = np.array(bezier_curve_path_3nd(x_base, images_tmp2, p, q, ens=int(FLAGS.ens)))
                        elif 'Catmull-Rom' in path_name:
                            path = np.array(catmull_rom_interpolation(x_base, images_tmp2, ens=int(FLAGS.ens)))
                        elif 'B_spline' in path_name:
                            path = np.array(bspline_interpolation(x_base, images_tmp2, ens=int(FLAGS.ens)))
                        
                        for i in range(path.shape[1]):
                            images_tmp2 = path[:,i,:,:,:]
                            assert images_tmp2.shape[1:] == (int(FLAGS.image_size), int(FLAGS.image_size), 3)
