...
tf.flags.DEFINE_string('attack_method', 'SAM_NAA', 'The name of attack method.')
...
tf.flags.DEFINE_string('output_dir', './adv/SAM_NAA/', 'Output directory with images.')
...
"""parameter for SAM"""
tf.flags.DEFINE_float('lamda', 1.0, 'The lamda parameter')
...
def get_SAM_loss(opt_operations, weights, base_feature, lamda):
    loss = 0
    gamma = 1.0
    for layer in opt_operations:
        ...
        # weights = IA   (y - y') * IA
        # n = adv_tensor.shape[1] * adv_tensor.shape[2] - 1
        # # square of (t - u)
        # d = tf.pow((adv_tensor - tf.reduce_mean(adv_tensor, axis=(1, 2), keepdims=True)), 2)
        # # d.sum() / n is channel variance
        # v = tf.reduce_mean(d, axis=(1, 2), keepdims=True) / tf.cast(n, tf.float32)
        # inv = d / (4 * (v + lamda)) + 0.5
        # attribution = (adv_tensor * inv - base_feature) * weights
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        # loss = WAy
        loss = get_SAM_loss(opt_operations, weights_ph, base_feature, FLAGS.lamda)
